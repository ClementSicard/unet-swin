...
def run(train_path: str, val_path: str, test_path: str):
    # Load data
    append_to_log("Loading data...")
    train_images, train_masks, val_images, val_masks = load_data(train_path, val_path)

    append_to_log(f"\tTraining images: {train_images.shape[0]}")
    append_to_log(f"\tTraining masks: {train_masks.shape[0]}")
    append_to_log(f"\tValidation images: {val_images.shape[0]}")
    append_to_log(f"\tValidation masks: {val_masks.shape[0]}")

    append_to_log("Creating patches...")
    ...
    # Extract features
    append_to_log("Extracting features...")
    x_train = extract_features(train_patches)
    x_val = extract_features(val_patches)

    append_to_log("Training model...")
    # Train model
    model = LinearSVC().fit(x_train, train_labels)
    append_to_log(f"\tTraining accuracy: {model.score(x_train, train_labels)}")
    append_to_log(f"Validation accuracy: {model.score(x_val, val_labels)}")
